...
class FrontController extends Controller
{
    public function index(Request $request)
    {
        ...
        //     ->where(['status' => 1])
        //     ->get();
        $result['users'] = DB::table('products')->get();
        // $result['home_categories'] = DB::table('categories')
        //     ->where(['status' => 1])
        //     ->where(['is_home' => 1])
        //     ->get();
    ...
    public function products()
    {
        ...
        $result['all_category'] = DB::table('categories')
            ->where(['status' => 1])
            // ->where(['is_home' => 1])
            ->get();


        foreach ($result['all_category'] as $list) {
            $result['all_category_product'][$list->id] =
                DB::table('products')
                ->where(['status' => 1])
                ->where(['category_id' => $list->id])
                ->get();
        }
        // $result['all_category_product'] =
        //     DB::table('products')
        //     ->where(['status' => 1])
        //     // ->where(['is_featured' => 1])
        //     ->get();
        //  prx($result['all_category_product']);
    ...
    public function add_to_cart(Request $request)
    {
        ...
        $result = DB::table('cart')
            ...
            ->where(['user_id' => $uid])
            ->where(['user_type' => $user_type])
            ->select('cart.qty', 'products.name', 'products.folder_name',  'products_attr.price', 'products.slug', 'products.id as pid', 'products_attr.id as attr_id')
            ->get();
        // dd($result);
